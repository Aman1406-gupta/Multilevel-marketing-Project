...
exports.ser_registeruser=async (req,rep)=>{
    ...
    if(previousdata.email){
        ...
        let parent_wallet=await recu.findOne({email:parentmail},{amount_earned:1})
        if(userd.parentmail){
...
exports.ser_showuser=async(req,rep)=>{
    let userdata=await recu.findOne({parentmail:rootmail},{});
...
exports.ser_add_balance_form=async(req,rep)=>{
    let user_info=await recu.findOne({email:"r@gmail.com"},{})
...
exports.ser_add_balance=async(req,rep)=>{
    ...
    if(v){
        await recu.findOneAndUpdate({email:"r@gmail.com"},{amount_earned:new_amount})
